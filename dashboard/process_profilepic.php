...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
  	...
  	if ($pad) {
  		$imagick->resizeImage(50,0,imagick::FILTER_LANCZOS, 1);
  		$imagick->setImageBackgroundColor('white');
  		$imagick->extentImage(50,50, 0, -(2048 - $imagick->getImageHeight())/2);
  	} else {
  	...
  	$imagick->writeImage($target_file);
		...
		$result = $s3Client->putObject(array(
		    'Bucket'     => $bucket,
		    'Key'        => $uploader.'/profile/'.$file_basename,
		...
		$plainUrl = $s3Client->getObjectUrl($bucket, $uploader.'/profile/'.$file_basename);

		if ($insert_stmt = $contentmysqli->prepare("INSERT INTO panorabbit_profilepic (user_id, profilepic_url) VALUES (?, ?)
			ON DUPLICATE KEY UPDATE profilepic_url = ?")) {
		  $insert_stmt->bind_param('iss', $uploaderid, $plainUrl, $plainUrl);
